...
// -2 => help
public :
int parser(string msg,int& screenPos){                                         // FUNCTION FOR PARSING THE INPUT
    ...
    else if(msg.find("bin")!=string::npos)                 // CHECKING FOR firstOne FUNCTION
    {
      brack1 = msg.find("(") ;
      brack2 = msg.find(")") ;
      ...
      {
        num = atoi(msg.substr(brack1+1,brack2).c_str()) ;
       char * res=decimalToBin(num) ;
       mvprintw(++screenPos,0,"%s",res);
       return PRINTED;
      }
      else
        return -1 ;
...
int main(){
  ...
  while(1){
    ...
    while(c!=10)
    {
      if((c>=65 && c<=90) || (c>=97 && c<=122) || c == 40 || c == 41||c==44 || (c>=48 && c<=57))
      {
        msg.push_back(c) ;
      }
      
      else if((c == KEY_LEFT)||c==KEY_BACKSPACE)
      {
        ...
        getyx(stdscr, cursy, cursx) ;
		 mvprintw(cursy,cursx,"%s",msg.c_str());
    ...
    if(msg.empty()){                    // Check for Enter Key
        ...
        status = obj.parser(msg,i) ;
        //just in case the object needs to dump
        //a cleaner solution would be another class but this would also
        //work
      if(status!=-1){
        if(status == -2){                               // Help Detected
		  ...
		  mvprintw(++i, 0, "bin(x) : Prints the binary representation of x)") ;
        
        }
        else if(status==obj.PRINTED)
        {
			//noop
		}
		else{
          mvprintw(++i, 0, " %u", status) ;}
      }
      else
